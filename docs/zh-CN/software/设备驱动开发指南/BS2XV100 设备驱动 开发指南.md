...
<tbody><tr id="row44971270352"><td class="cellrowborder" valign="top" width="18.7%" headers="mcps1.1.4.1.1 "><p id="p0498927143517"><a name="p0498927143517"></a><a name="p0498927143517"></a>06</p>
</td>
...
<td class="cellrowborder" valign="top" width="61.06%" headers="mcps1.1.4.1.3 "><a name="ul15657113573710"></a><a name="ul15657113573710"></a><ul id="ul15657113573710"><li>更新“<a href="Pinctrl.md">Pinctrl</a>”的“<a href="注意事项.md">注意事项</a>”章节内容。</li><li>更新“<a href="UART.md">UART</a>”的“<a href="KCONFIG配置-12.md">KCONFIG配置</a>”章节内容。</li><li>更新“<a href="SPI.md">SPI</a>”的“<a href="开发指引-18.md">开发指引</a>”章节内容。</li></ul>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61.06%" headers="mcps1.1.4.1.3 "><a name="ul63460595179"></a><a name="ul63460595179"></a><ul id="ul63460595179"><li>更新“<a href="Pinctrl.md">Pinctrl</a>”章节内容。</li><li>更新“<a href="SPI.md">SPI</a>”章节内容。</li><li>更新“<a href="I2C.md">I2C</a>”的“<a href="KCONFIG配置-22.md">KCONFIG配置</a>”章节内容。</li><li>更新“<a href="ADC.md">ADC</a>”章节内容。</li><li>更新“<a href="Flash.md">Flash</a>”章节内容。</li><li>更新“<a href="PDM.md">PDM</a>”章节内容。</li></ul>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="61.06%" headers="mcps1.1.4.1.3 "><a name="ul615755620593"></a><a name="ul615755620593"></a><ul id="ul615755620593"><li>更新“<a href="Pinctrl.md">Pinctrl</a>”章节内容。</li><li>更新“<a href="GPIO.md">GPIO</a>”章节内容。</li><li>更新“<a href="UART.md">UART</a>”章节内容。</li><li>更新“<a href="SPI.md">SPI</a>”章节内容。</li><li>更新“<a href="ADC.md">ADC</a>”章节内容。</li><li>更新“<a href="DMA.md">DMA</a>”的“<a href="注意事项-34.md">注意事项</a>”章节内容。</li><li>更新“<a href="Flash.md">Flash</a>”的“<a href="功能描述-61.md">功能描述</a>”章节内容。</li><li>更新“<a href="KEYSCAN.md">KEYSCAN</a>”的“<a href="开发指引-77.md">开发指引</a>”章节内容。</li></ul>
</td>
</tr>
...
# CAN（仅限BS21A使用）<a name="ZH-CN_TOPIC_0000001893716542"></a>
...
## 概述<a name="ZH-CN_TOPIC_0000001893876426"></a>

CAN（Control Area Network）是一种车用总线标准，其被设计用于在不需要主机（Host）的情况下，允许网络上的单片机和仪器相互通信。

CAN规格如下：

-   支持配置CAN速率、采样点、滤波器。
-   支持Classic CAN与CAN-FD格式报文传输。
-   支持上报CAN节点状态中断。

## 功能描述<a name="ZH-CN_TOPIC_0000001935836137"></a>

CAN模块提供的接口及功能如下：

-   uapi\_can\_init（uint8\_t bus, can\_global\_config\_t \*global\_config, can\_filter\_list\_t \*std\_filter,  can\_filter\_list\_t \*ex\_filter）：初始化CAN控制器（bus表示当前CAN总线，global\_config表示CAN-FD全局配置，包含波特率和工作模式配置，std\_filter表示CAN-FD标准标识符滤波配置，ex\_filter表示CAN-FD扩展标识符滤波配置），初始化时，默认配置CAN-FD TDC与数据段采样点前的长度一致。
-   uapi\_can\_deinit（uint8\_t bus）：去初始化CAN控制器（bus表示当前CAN总线）。
-   uapi\_can\_send\_msg（uint8\_t bus, can\_msg\_attr\_t \*can\_msg, uint8\_t \*data, uint8\_t length）：发送CAN报文（bus表示当前CAN总线，can\_msg表示预计发送的CAN数据帧和远程帧的仲裁域，data表示预计发送的数据，length表示预计发送的数据长度）。
-   uapi\_can\_receive\_msg（uint8\_t bus, uint8\_t fifo\_index, can\_msg\_attr\_t \*receive\_msg, uint8\_t \*data, uint32\_t \*length）：读取FIFO中的CAN报文（bus表示当前CAN总线，fifo\_index表示接收fifo的索引，receive\_msg表示预计接收到的CAN帧仲裁域，data表示接受数据的缓冲区，length表示接受到的数据真实长度）。
-   uapi\_can\_callback\_register（uint8\_t bus, can\_callback\_t can\_callback）：注册CAN中断回调函数（bus表示当前CAN总线，can\_callback表示当前CAN的中断回调函数）。
-   uapi\_can\_callback\_unregister（uint8\_t bus）：去注册CAN中断回调函数（bus表示当前CAN总线）。
-   uapi\_can\_get\_controller\_sts（uint8\_t bus）：获取CAN控制器状态（bus表示当前CAN总线）。
-   uapi\_can\_get\_transmit\_error\_count（uint8\_t bus）：获取CAN控制器发送错误计数（bus表示当前CAN总线）。
-   uapi\_can\_get\_receive\_error\_count（uint8\_t bus）：获取CAN控制器接收错误计数（bus表示当前CAN总线）。
-   uapi\_can\_fd\_config\_tdc\(uint8\_t bus, uint8\_t enable, uint8\_t offset\) : 配置CAN-FD发送延迟补偿偏移，offset单位为TQ，BS21A的场景下TQ以40MHz时钟为标准。

## KCONFIG配置<a name="ZH-CN_TOPIC_0000001896046754"></a>

![](figures/zh-cn_image_0000002158944608.png)
...
<th class="cellrowborder" valign="top" width="14.398560143985604%" id="mcps1.2.7.1.6"><p id="p1162923893911"><a name="p1162923893911"></a><a name="p1162923893911"></a>是否选择打开或修改配置</p>
</th>
</tr>
</thead>
<tbody><tr id="row16629338103916"><td class="cellrowborder" valign="top" width="22.62773722627738%" headers="mcps1.2.7.1.1 "><p id="p166291338133910"><a name="p166291338133910"></a><a name="p166291338133910"></a>CONFIG_CAN_BUS_NUM</p>
</td>
<td class="cellrowborder" valign="top" width="21.137886211378866%" headers="mcps1.2.7.1.2 "><p id="p106291638163910"><a name="p106291638163910"></a><a name="p106291638163910"></a>Number of CAN controller bus.</p>
</td>
<td class="cellrowborder" valign="top" width="19.878012198780127%" headers="mcps1.2.7.1.3 "><p id="p148018553156"><a name="p148018553156"></a><a name="p148018553156"></a>配置CAN通路数，本产品仅有一路CAN控制器，使用默认配置1</p>
</td>
<td class="cellrowborder" valign="top" width="10.618938106189384%" headers="mcps1.2.7.1.4 "><p id="p166297382397"><a name="p166297382397"></a><a name="p166297382397"></a>是</p>
</td>
<td class="cellrowborder" valign="top" width="11.338866113388663%" headers="mcps1.2.7.1.5 "><p id="p162983873917"><a name="p162983873917"></a><a name="p162983873917"></a>否</p>
</td>
<td class="cellrowborder" valign="top" width="14.398560143985604%" headers="mcps1.2.7.1.6 "><p id="p142312269162"><a name="p142312269162"></a><a name="p142312269162"></a>默认值是1，建议不做修改</p>
</td>
</tr>
<tr id="row166299383394"><td class="cellrowborder" valign="top" width="22.62773722627738%" headers="mcps1.2.7.1.1 "><p id="p14629173813920"><a name="p14629173813920"></a><a name="p14629173813920"></a>CONFIG_STANDARD_CAN_FILTER_NUM</p>
</td>
<td class="cellrowborder" valign="top" width="21.137886211378866%" headers="mcps1.2.7.1.2 "><p id="p130716441711"><a name="p130716441711"></a><a name="p130716441711"></a>Maximum number of standard CAN filter.</p>
</td>
<td class="cellrowborder" valign="top" width="19.878012198780127%" headers="mcps1.2.7.1.3 "><p id="p1373182005115"><a name="p1373182005115"></a><a name="p1373182005115"></a>配置CAN 滤波器个数，驱动最大支持128个，本产品默认地址分配最大支持16个</p>
</td>
<td class="cellrowborder" valign="top" width="10.618938106189384%" headers="mcps1.2.7.1.4 "><p id="p166291838143916"><a name="p166291838143916"></a><a name="p166291838143916"></a>是</p>
</td>
<td class="cellrowborder" valign="top" width="11.338866113388663%" headers="mcps1.2.7.1.5 "><p id="p2629138193917"><a name="p2629138193917"></a><a name="p2629138193917"></a>否</p>
</td>
<td class="cellrowborder" valign="top" width="14.398560143985604%" headers="mcps1.2.7.1.6 "><p id="p126297381398"><a name="p126297381398"></a><a name="p126297381398"></a>默认值是10，根据实际情况配置，建议不做修改</p>
</td>
</tr>
<tr id="row3629193823919"><td class="cellrowborder" valign="top" width="22.62773722627738%" headers="mcps1.2.7.1.1 "><p id="p262983811395"><a name="p262983811395"></a><a name="p262983811395"></a>CONFIG_EXTENDED_CAN_FILTER_NUM</p>
</td>
<td class="cellrowborder" valign="top" width="21.137886211378866%" headers="mcps1.2.7.1.2 "><p id="p1173515130188"><a name="p1173515130188"></a><a name="p1173515130188"></a>Maximum number of extend CAN filter.</p>
</td>
<td class="cellrowborder" valign="top" width="19.878012198780127%" headers="mcps1.2.7.1.3 "><p id="p117352016518"><a name="p117352016518"></a><a name="p117352016518"></a>配置CAN 扩展滤波器个数，驱动最大支持64个，本产品默认地址分配最大支持8个</p>
</td>
<td class="cellrowborder" valign="top" width="10.618938106189384%" headers="mcps1.2.7.1.4 "><p id="p3629133873917"><a name="p3629133873917"></a><a name="p3629133873917"></a>是</p>
</td>
<td class="cellrowborder" valign="top" width="11.338866113388663%" headers="mcps1.2.7.1.5 "><p id="p062910387397"><a name="p062910387397"></a><a name="p062910387397"></a>否</p>
</td>
<td class="cellrowborder" valign="top" width="14.398560143985604%" headers="mcps1.2.7.1.6 "><p id="p1055216012245"><a name="p1055216012245"></a><a name="p1055216012245"></a>默认值是10，根据实际情况配置，建议不做修改</p>
</td>
</tr>
<tr id="row962920381394"><td class="cellrowborder" valign="top" width="22.62773722627738%" headers="mcps1.2.7.1.1 "><p id="p1629738173912"><a name="p1629738173912"></a><a name="p1629738173912"></a>CONFIG_CAN_TX_BUFFER_SIZE</p>
</td>
<td class="cellrowborder" valign="top" width="21.137886211378866%" headers="mcps1.2.7.1.2 "><p id="p1286173422617"><a name="p1286173422617"></a><a name="p1286173422617"></a>Tx FIFO maximum size.</p>
</td>
<td class="cellrowborder" valign="top" width="19.878012198780127%" headers="mcps1.2.7.1.3 "><p id="p1019944663711"><a name="p1019944663711"></a><a name="p1019944663711"></a>配置Tx FIFO深度，驱动最大支持32个，本产品默认地址分配最大支持8个</p>
</td>
<td class="cellrowborder" valign="top" width="10.618938106189384%" headers="mcps1.2.7.1.4 "><p id="p7630938143910"><a name="p7630938143910"></a><a name="p7630938143910"></a>是</p>
</td>
<td class="cellrowborder" valign="top" width="11.338866113388663%" headers="mcps1.2.7.1.5 "><p id="p56301938153913"><a name="p56301938153913"></a><a name="p56301938153913"></a>否</p>
</td>
<td class="cellrowborder" valign="top" width="14.398560143985604%" headers="mcps1.2.7.1.6 "><p id="p19630238203917"><a name="p19630238203917"></a><a name="p19630238203917"></a>默认值是8，根据实际情况配置，建议不做修改</p>
</td>
</tr>
<tr id="row62681237271"><td class="cellrowborder" valign="top" width="22.62773722627738%" headers="mcps1.2.7.1.1 "><p id="p112401826112812"><a name="p112401826112812"></a><a name="p112401826112812"></a>CONFIG_CAN_RX_FIFO0_SIZE</p>
</td>
<td class="cellrowborder" valign="top" width="21.137886211378866%" headers="mcps1.2.7.1.2 "><p id="p73358962919"><a name="p73358962919"></a><a name="p73358962919"></a>Rx FIFO 0 maximum size.</p>
</td>
<td class="cellrowborder" valign="top" width="19.878012198780127%" headers="mcps1.2.7.1.3 "><p id="p5199125485910"><a name="p5199125485910"></a><a name="p5199125485910"></a>配置Rx FIFO 0深度，驱动最大支持64个，内部用作接收Buffer，本产品默认地址分配最大支持10个</p>
</td>
<td class="cellrowborder" valign="top" width="10.618938106189384%" headers="mcps1.2.7.1.4 "><p id="p5269202314277"><a name="p5269202314277"></a><a name="p5269202314277"></a>是</p>
</td>
<td class="cellrowborder" valign="top" width="11.338866113388663%" headers="mcps1.2.7.1.5 "><p id="p2269122314274"><a name="p2269122314274"></a><a name="p2269122314274"></a>否</p>
</td>
<td class="cellrowborder" valign="top" width="14.398560143985604%" headers="mcps1.2.7.1.6 "><p id="p12269623182717"><a name="p12269623182717"></a><a name="p12269623182717"></a>默认值是10，根据实际情况配置，建议不做修改</p>
</td>
</tr>
<tr id="row17817572040"><td class="cellrowborder" valign="top" width="22.62773722627738%" headers="mcps1.2.7.1.1 "><p id="p109831846175713"><a name="p109831846175713"></a><a name="p109831846175713"></a>CONFIG_CAN_RX_FIFO1_SIZE</p>
</td>
<td class="cellrowborder" valign="top" width="21.137886211378866%" headers="mcps1.2.7.1.2 "><p id="p36957616613"><a name="p36957616613"></a><a name="p36957616613"></a>Rx FIFO 1 maximum size.</p>
</td>
<td class="cellrowborder" valign="top" width="19.878012198780127%" headers="mcps1.2.7.1.3 "><p id="p177555421103"><a name="p177555421103"></a><a name="p177555421103"></a>配置Rx FIFO 1深度，驱动最大支持64个，内部用作接收Buffer，本产品默认地址分配最大支持10个</p>
</td>
<td class="cellrowborder" valign="top" width="10.618938106189384%" headers="mcps1.2.7.1.4 "><p id="p17919571246"><a name="p17919571246"></a><a name="p17919571246"></a>是</p>
</td>
<td class="cellrowborder" valign="top" width="11.338866113388663%" headers="mcps1.2.7.1.5 "><p id="p79175714418"><a name="p79175714418"></a><a name="p79175714418"></a>否</p>
</td>
<td class="cellrowborder" valign="top" width="14.398560143985604%" headers="mcps1.2.7.1.6 "><p id="p85412518116"><a name="p85412518116"></a><a name="p85412518116"></a>默认值是10，根据实际情况配置，建议不做修改</p>
</td>
</tr>
</tbody>
</table>

## 开发指引<a name="ZH-CN_TOPIC_0000001893716558"></a>

CAN接口遵循如下操作步骤：

1.  调用uapi\_pin\_init接口，初始化PINCTRL模块。
2.  调用uapi\_pin\_set\_mode接口，将PIN复用为CAN\_TX及CAN\_RX。
3.  调用uapi\_can\_init接口，初始化CAN模块，配置CAN控制器的传输速率等参数。
4.  调用uapi\_can\_callback\_register接口，注册CAN中断回调，默认采用中断模式接收CAN网络上的报文。
5.  调用uapi\_can\_send\_msg接口，发送CAN报文。
...
## 注意事项<a name="ZH-CN_TOPIC_0000001893876434"></a>

-   驱动内部可以自行自动重发，可以对can\_porting\_get\_timeout\_duration的返回值进行修改以调整最大重发时长。
-   实际发送报文时将遵循CAN-FD对DLC的定义，不足的长度将自动补0处理。
-   默认使用中断模式接收报文，receive接口作为冗余接口保留。
-   初始化时将对CAN控制器进行复位，去初始化后重新初始化时，控制器内部的所有配置和报文会丢失。
-   收发Buffer使用的地址空间为0x58020000 \~ 0x58023FFF（CAN控制器私有空间），用户可于can\_porting.c中自由配置该段地址空间以增加Buffer与filter的地址空间。配合KConfig可进行对Buffer和filter的自由配置。
-   不提供CAN Tranceiver，根据CAN Tranceiver的不同可能需要额外使用GPIO控制Tranceiver，驱动内部不对此部分进行处理。对CAN 控制器进行调试，可以将初始化参数 global\_config的成员can\_mode配置为LOOPBACK\_MODE，此时内部将进行回环发送，发送的报文将从CAN Tx对应的IO发送。
-   CAN驱动通信失败时，可将驱动初始化为回环模式进行自收发调试，若回环模式可发送，请检查在正常工作模式下的IO配置\(配置错误管脚，配置了多组Tx/Rx，Rx输入使能配置\)。若回环模式不可发送，请检查初始化函数返回值以确认是否正确套用了预期的参数配置。回环模式可以通过TX经过tranceiver发送至总线，可对接CAN测试仪器等设备辅助测试。
...
-   uapi\_uart\_write（uart\_bus\_t bus, const uint8\_t \*buffer, uint32\_t length, uint32\_t timeout）：将数据发送到已经打开的UART上，使用直接发送的方式（bus表示当前UART bus串口号，buffer表示要发送的数据Buffer，length表示要发送的数据Buffer长度，timeout表示超时时间，该参数暂未使用\(需开启CONFIG\_SUPPORT\_UART\_TX\_POLL\_TIMEOUT, timeout参数才生效\)）。（打开CONFIG\_UART\_SUPPORT\_TX宏才能使用）
-   uapi\_uart\_write\_nolock（uart\_bus\_t bus, const uint8\_t \*buffer, uint32\_t length, uint32\_t timeout）：将数据发送到已经打开的UART上，使用直接发送不会锁中断的方式（bus表示当前UART bus串口号，buffer表示要发送的数据Buffer，length表示要发送的数据Buffer长度，timeout表示超时时间，该参数暂未使用\(需开启CONFIG\_SUPPORT\_UART\_TX\_POLL\_TIMEOUT, timeout参数才生效\)）。（打开CONFIG\_UART\_SUPPORT\_TX宏才能使用）
...
<td class="cellrowborder" valign="top" width="19.878012198780123%" headers="mcps1.2.7.1.3 "><p id="p144832103113"><a name="p144832103113"></a><a name="p144832103113"></a>UART TX接口是否打开超时机制</p>
</td>
<td class="cellrowborder" valign="top" width="10.608939106089391%" headers="mcps1.2.7.1.4 "><p id="p1044133210317"><a name="p1044133210317"></a><a name="p1044133210317"></a>是</p>
</td>
...
<td class="cellrowborder" valign="top" width="14.3985601439856%" headers="mcps1.2.7.1.6 "><p id="p184413243117"><a name="p184413243117"></a><a name="p184413243117"></a>默认关闭，视情况选择</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="19.878012198780123%" headers="mcps1.2.7.1.3 "><p id="p868784017376"><a name="p868784017376"></a><a name="p868784017376"></a>UART RX接口是否开启超时机制</p>
</td>
<td class="cellrowborder" valign="top" width="10.608939106089391%" headers="mcps1.2.7.1.4 "><p id="p106876408372"><a name="p106876408372"></a><a name="p106876408372"></a>是</p>
</td>
...
<td class="cellrowborder" valign="top" width="14.3985601439856%" headers="mcps1.2.7.1.6 "><p id="p14687194073718"><a name="p14687194073718"></a><a name="p14687194073718"></a>默认关闭，视情况选择</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="19.878012198780123%" headers="mcps1.2.7.1.3 "><p id="p2865183705310"><a name="p2865183705310"></a><a name="p2865183705310"></a>UART支持DMA模式，视情况选择，若选择采用UART DMA模式，则需同步选择DMA驱动的KCONFIG配置，请参考“<a href="KCONFIG配置-32.md">KCONFIG配置</a>”</p>
</td>
...
<td class="cellrowborder" valign="top" width="19.878012198780123%" headers="mcps1.2.7.1.3 "><p id="p148599438"><a name="p148599438"></a><a name="p148599438"></a>UART支持在DMA中断里发送数据，视情况选择，若选择采用UART DMA模式，则需同步选择DMA驱动的KCONFIG配置，请参考“<a href="KCONFIG配置-32.md">KCONFIG配置</a>”</p>
</td>
...
<td class="cellrowborder" valign="top" width="19.938006199380062%" headers="mcps1.2.7.1.3 "><p id="p1164428144915"><a name="p1164428144915"></a><a name="p1164428144915"></a>SPI支持DMA模式，视情况选择，若选择采用SPI DMA模式，则需同步选择DMA驱动的KCONFIG配置，请参考“<a href="KCONFIG配置-32.md">KCONFIG配置</a>”。</p>
</td>
<td class="cellrowborder" valign="top" width="10.608939106089391%" headers="mcps1.2.7.1.4 "><p id="p3629133873917"><a name="p3629133873917"></a><a name="p3629133873917"></a>是</p>
</td>
...
<td class="cellrowborder" valign="top" width="19.878012198780123%" headers="mcps1.2.7.1.3 "><p id="p1164428144915"><a name="p1164428144915"></a><a name="p1164428144915"></a>I2C支持DMA模式，视情况选择，若选择采用I2C DMA模式，则需同步选择DMA驱动的KCONFIG配置，请参考<a href="KCONFIG配置-32.md">KCONFIG配置</a></p>
</td>
<td class="cellrowborder" valign="top" width="10.608939106089391%" headers="mcps1.2.7.1.4 "><p id="p17919571246"><a name="p17919571246"></a><a name="p17919571246"></a>是</p>
</td>
...
-   AAFE采集音频信号处理方法。
    ...
    通过uapi\_adc\_auto\_sample采集到的音频信号与GAFE采集到的电量信号格式相同，包含13bit有效位及1bit符号位，处理完的值通过PDM模块发送，具体实现参考“[开发指引](开发指引-82.md)”章节。
...
**新选型的外部Flash适配，需要在drivers/chips/bs2x/porting/flash/flash\_config\_info\_porting.h中维护全局变量g\_flash\_device\_parameter，该全局变量的结构体为flash\_device\_parameter\_t：**
...
-   manufacturer\_id：在单线模式下即可读取，发送0x9F读取id，通过手册中查找，对比两种方式得到的结果是否一致。下面列举一些常见厂商的manufacturer\_id位置 。
...
-   security\_reg\_cmd：读取Flash状态寄存器的指令，查找手册中的Read Security Registers，可在Commands Table中查到。
-   suspend\_cmd和resume\_cmd：支持休眠的Flash，手册中搜索suspend和resume，可在Commands Table中查到。

    **drivers/chips/bs2x/porting/flash/flash\_config\_info\_porting.h中还维护了一个全局变量g\_flash\_need\_set\_qe，该全局变量保存了使能四线访问前需要开启QE Bit的FLash的manufacturer id，以及开启QE Bit的操作，是否需要开启QE Bit，可以在手册中搜索Quad Enable \(QE\)。**

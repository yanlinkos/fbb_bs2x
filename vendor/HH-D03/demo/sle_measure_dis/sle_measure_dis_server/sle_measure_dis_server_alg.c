...
void measure_dis_print_cal_dis(float dist_first, float dist_ori, float prob, uint32_t time)
{
    ...
    osal_printk("SLEM get distance done. distance:%d.%03d, time = %d ms. \r\n", (int32_t)(dist_first),
                ((uint32_t)(dist_first * MEASURE_DIS_NUM_CARRY_1000) % MEASURE_DIS_NUM_CARRY_1000), (uint32_t)(time));
    ssd1306_SetCursor(0, 0);
    int ret = sprintf(templine, "distance: %d.%03d", (int32_t)(dist_first),
                      ((uint32_t)(dist_first * MEASURE_DIS_NUM_CARRY_1000) % MEASURE_DIS_NUM_CARRY_1000));
    if (ret < 0) {
        printf("humi failed\r\n");
    }
                                     ...
                                     sle_channel_sounding_iq_trans_t *remote_iq_data)
{
    ...
    slem_para_pair para_limit = {
        .rssi_limit = DIS_ALG_RSSI_LIMIT, .threshold_cond2 = DIS_ALG_THRESHOULD_COND2, .r_start = DIS_ALG_R_START};
    ...
    measure_dis_print_cal_dis(result_dist.dis_smoothed, result_dist.dis_ori, result_dist.prob, time_end - time_start);
...
void measure_dis_print_iq_data(uint8_t num, uint8_t role, sle_channel_sounding_iq_trans_t *report)
{
    ...
    osal_printk("[iq_data]%d,%d,%d,%d,0x%x,", num, role, report->timestamp_sn, report->samp_cnt, report->rssi);

...
{
    alg_para->calib_val = 0; /* 根据5次测量平均值0.8来校准 */
    alg_para->tof_calib = TOF_DEFAULT;
    alg_para->ranging_method = DIS_ALG_MODE;
}
...
{
    slem_alg_para_dis *alg_para = (slem_alg_para_dis *)osal_kmalloc(sizeof(slem_alg_para_dis), 0);/*使用osal_kmalloc为alg_para_dis结构体分配内存*/
...
#endif
    alg_para->iq_rtd = (slem_alg_iq *)&(g_md_local_iq_data.data[0]); /*填充特定参数*/
    ...
    uint32_t time_start = (uint32_t)uapi_tcxo_get_ms(); /*配置时钟和测量时间*/

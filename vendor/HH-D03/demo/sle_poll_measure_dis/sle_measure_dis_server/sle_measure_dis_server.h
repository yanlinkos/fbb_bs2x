...
errcode_t sle_measure_dis_msg_add(measure_dis_msg_node_t *msg);/*用于将测量距离相关的消息节点添加到某个消息队列或处理列表中，以便后续处理。它接受一个指向measure_dis_msg_node_t结构体的指针作为参数。*/
void sle_measure_dis_msg_proc(void);/*负责处理消息队列中的测量距离消息*/
int sle_measure_recv_dis(uint8_t *value, uint16_t len);/*用于接收测量距离的数据*/

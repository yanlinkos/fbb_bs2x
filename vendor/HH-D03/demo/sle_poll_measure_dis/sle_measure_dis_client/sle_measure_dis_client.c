...
uint8_t g_measure_dis_conn_id[MAX_SERVERS] = {SLEM_CONNET_INVAILD}; 
...
errcode_t measure_dis_slem_set_param(uint16_t conn_id)
{
    sle_set_channel_sounding_param_ex_t param = {
        .acb_interval = 0,   
        .con_anchor_num = 3,  /*表示连接锚点（Connection Anchor）的数量*/
        .cs_interval = 0,     /*代表信道探测（Channel Sounding）的间隔*/
        .freq_space = 0,      /*频率间隔*/
...
static int measure_dis_client_discover_key(uint8_t type)
{
    ...
    {
        uint8_t conn_id = g_measure_dis_conn_id[i];
        if (conn_id != SLEM_CONNET_INVAILD)
        {
            ...
            {
                memcpy_s(param.uuid.uuid, SLEM_UUID_LEN, g_measure_dis_server_data.service_uuid, SLEM_UUID_LEN);
            }
            else if (type == SSAP_FIND_TYPE_PROPERTY)
            {
                memcpy_s(param.uuid.uuid, SLEM_UUID_LEN, g_measure_dis_server_data.property_uuid, SLEM_UUID_LEN);
            }
            osal_printk("SLE START CLIENT DISCOVER SERVER. conn_id:%d \r\n", conn_id);
            ssapc_find_structure(0, conn_id, &param); 
        }
    }
    return ERRCODE_SUCC;
}
                                          ...
                                          sle_pair_state_t pair_state, sle_disc_reason_t disc_reason)
{
    ...
    if (conn_state == SLE_ACB_STATE_CONNECTED)
    {
        ...
        g_measure_dis_conn_id[g_connected_count] = conn_id;
        g_connected_count++;
        osal_printk("Connected to Server %d, conn_id: %d\n", g_connected_count, conn_id);
        ...
        // 如果还可以连接更多 Server，则重新启动扫描
        if (g_connected_count < MAX_SERVERS)
        {
            osal_printk("Re-start scanning for next Server...\n");
    ...
    else if (conn_state == SLE_ACB_STATE_DISCONNECTED)
    {
        ...
        for (int i = 0; i < MAX_SERVERS; i++)
        {
            if (g_measure_dis_conn_id[i] == conn_id)
            {
                ...
                g_connected_count--;
                osal_printk("Disconnected from Server %d, conn_id: %d\n", i, conn_id);
                break;
            }
        }
        ...
        if (g_connected_count < MAX_SERVERS)
        {
            osal_printk("Re-start scanning after disconnection...\n");
            measure_dis_start_scan(); 
        }
    }
}
...
static void measure_dis_dd_seek_result_cbk(sle_seek_result_info_t *seek_result_data) 
{
    ...
    if (g_connected_count < MAX_SERVERS)
    {
        osal_printk("Connecting to Server with address: ");
        for (int i = 0; i < SLE_ADDR_LEN; i++)
        {
            osal_printk("%02X ", seek_result_data->addr.addr[i]);
        }
        osal_printk("\n");

        sle_connect_remote_device(&(seek_result_data->addr)); // 连接当前扫描到的 Server
    }
}

errcode_t measure_dis_dm_register_cbks(void) /*设备管理回调函数*/
{
...
errcode_t measure_dis_dd_register_cbks(void) /*扫描回调注册*/
{
...
STATIC void measure_dis_ssapc_find_structure_cbk(uint8_t client_id, uint16_t conn_id,
                                                 ssapc_find_service_result_t *service, errcode_t status) /*用于处理服务搜索的结果的回调函数*/
{
...
void measure_dis_client_msg_proc(uint8_t *data, uint16_t data_len) /*处理来自测量设备或服务的客户端消息*/
{
    unused(data_len);
    uint32_t ret = ERRCODE_SLE_FAIL;
    measure_ids_msg_t *slem_profile_msg = (measure_ids_msg_t *)(data); /*data 指针转换为 measure_ids_msg_t 类型的指针*/

    switch (slem_profile_msg->type)
    {
    case SLEM_PROFILE_MSG_IQ: /*当接收到IO数据时，函数不执行任何操作*/
        break;
    default:
        for (int i = 0; i < data_len; i++)
        {
            osal_printk("[%d]:%d\r\n", i, data[i]); /*当接收的不是IO数据时,打印出数据的每个字节（作为十进制数）。这有助于调试未知或不受支持的消息类型。*/
        }
        break;
    }
...
void measure_dis_protocol_stack_init(uint8_t *addr, uint8_t *name) /*注册回调函数 (设备管理、连接管理、扫描管理)并启动协议栈 (enable_sle)*/
{
...
int measure_dis_client_write_server(uint16_t conn_id, uint32_t type, uint8_t *data, uint32_t data_len)
{
    ...
    osal_printk("Broadcast IQ data to Server conn_id: %d\n", conn_id);

    measure_ids_msg_t *slem_msg = (measure_ids_msg_t *)osal_kmalloc(sizeof(measure_ids_msg_t) + data_len, 0);
    if (slem_msg == NULL)
    {
        ret = ERRCODE_MALLOC;
        return ret;
    ...
    memcpy_s(slem_msg->data, data_len, data, data_len);

    ret = measure_dis_client_write_proc(conn_id, 0, sizeof(measure_ids_msg_t) + data_len, (uint8_t *)slem_msg);
    if (ret != ERRCODE_SUCC)
    {
        osal_printk("Failed to send IQ data to Server conn_id: %d\n", conn_id);
    }
